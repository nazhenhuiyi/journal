...
当前核心已经有 `blocked` 状态和结构化 reason。桌面端和移动端会在同步设置页展示阻断入口；`content-conflict` 会展示冲突路径、可预览的本机/远端片段，并提供三个确认式操作。

| 优先级 | 要补的能力 | 完成标准 |
| --- | --- | --- |
| P0 | 端侧提示 | 设置页能显示“有内容冲突，需要处理后再同步”或“本机和远端不是同一条同步历史” |
| P1 | 冲突阻断恢复流程 | 已有首版：`content-conflict` 后展示冲突路径、当前影响和可选处理方式；用户处理完成后清除 blocked 状态 |
| P1 | 内容冲突处理入口 | 已有首版：用户能选择保留本机、保留远端或两者都保留 |
...
- 两者都保留：对冲突 Markdown 重新做三方合并，把无法自动判断的冲突段落展开为本机片段加远端片段，再创建解决冲突提交并正常 push；不会 force push。
