...
这份文档回答“照片地图自己的运行时对象有哪些，它们之间是什么关系，哪些只是临时派生”。它不定义日记原始模型；上游日记结构见 [数据结构](../数据结构.md)。它也不描述点击路径，交互见 [照片地图交互流转](照片地图交互流转.md)；不描述函数调用顺序，计算链路见 [照片地图数据流与渲染](照片地图数据流与渲染.md)。
...
| 可选位置 | 生成地图坐标、路线、相机和未定位计数 |

如果需要修改这些上游字段或持久化格式，应优先更新 [数据结构](../数据结构.md)，而不是在照片地图文档里重新定义一份。
...
| 未定位计数 | 正文无 murmur 坐标算文字未定位；图片无 image 坐标且无 murmur fallback 算图片未定位 |
...
| 想讨论的问题 | 推荐名词 |
| --- | --- |
| 日记里原始写了什么 | 上游日记模型，见 [数据结构](../数据结构.md) |
