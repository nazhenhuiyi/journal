...
这份文档记录移动端照片地图从日记数据到地图渲染的当前实现。代码事实以 `apps/mobile/src/pages/photoMapData.ts`、`apps/mobile/src/pages/photoMapInteraction.ts`、`apps/mobile/src/pages/photoMapViewModel.ts`、`apps/mobile/src/pages/usePhotoMapRuntime.ts`、`apps/mobile/src/pages/usePhotoMapInteractions.ts`、`apps/mobile/src/pages/PhotoMapMarkers.tsx`、`apps/mobile/src/pages/PhotoMapOverlays.tsx` 和 `apps/mobile/src/pages/PhotoMapPage.tsx` 为准；地图 UI 的目标是让用户在空间上回看文字碎碎念和照片，而不是把图片当作碎碎念的附属代表物。数据对象关系见 [照片地图数据模型](照片地图数据模型.md)，MapLibre 组件和事件机制见 [MapLibre 技术说明与依赖](<MapLibre 技术说明与依赖.md>)。
...
| 文件 | 职责 |
| --- | --- |
| `apps/mobile/src/pages/PhotoMapPage.tsx` | 页面壳、日记读取、MapLibre 容器和各子模块装配 |
...
| `apps/mobile/src/pages/usePhotoMapRuntime.ts` | 集中派生 slices、observations、clusters、lookup、统计数、active cluster 和相机 key |
| `apps/mobile/src/pages/usePhotoMapInteractions.ts` | 集中处理 marker 点击、cluster 展开、preview 恢复、地图空白、range 切换、recenter 和文字卡片轮播 |
| `apps/mobile/src/pages/PhotoMapMarkers.tsx` | 图片/文字 marker 与展开态 marker，统一 `event.stopPropagation()` |
| `apps/mobile/src/pages/PhotoMapOverlays.tsx` | 顶部统计卡、range menu、照片 tray、文字 sheet、底部 murmur 卡片和状态页 |
| `apps/mobile/src/pages/photoMapPresentation.ts` | range 文案、布局常量、marker offset、动画 helper 和展示格式化 |
| `apps/mobile/src/pages/photoMapStyles.ts` | 照片地图样式和视觉层级 token |
| `apps/mobile/src/pages/photoMapCamera.ts` | 初始相机移动适配 |
...
flowchart TD
  ...
  D --> S["getUnlocatedPhotoMapContentCount()<br/>状态/调试用未定位计数"]
...
- `PageShell` 只保留 header 区域的安全区、返回按钮和右侧范围入口；地图内容区通过 `photoMapShellContent.paddingHorizontal = 0` 横向拉通。
- `mapFrame` 本身仍保留圆角和边框，地图、marker、统计卡、tray、sheet 和底部卡片都在这个框内叠放。
- 文字卡片轮播的 item 宽度由地图实际宽度计算，右侧保留 `textCarouselPeekWidth = spacingPixels['8']` 的露出量；当前没有额外平台专用触摸拦截层，横滑只依赖 `FlatList` 的 snap 行为。

视觉层级当前遵循低噪声规则：

- 路线主线使用主题绿但透明度很低，阴影线使用浅色底，避免压过地图路网。
- 文字 marker 未选中时使用 `primary-soft` / `primary-hover`，选中或展开聚合时才使用主题绿实心。
- 图片 marker 固定向右上偏移，文字 marker 不偏移；同一位置有图文时，图片稍微让开文字点。
- 图片聚合 badge 使用中性白底和弱边框，不再用高饱和强调色。
- 顶部统计卡显示周期标题、碎碎念数和照片数，不展示未定位数量；定位按钮使用中性图标色。
- 底部 murmur 卡片正文使用较小字号、较低颜色和两行省略；`附近 N 条` badge 在 meta 行右侧，负责打开文字 cluster sheet。
...
| `range` | observation、cluster、路线、统计卡数量和内部未定位判断 |
...
`getUnlocatedPhotoMapContentCount()` 仍然保留在 view model 中，但它不再作为顶部统计卡的常规信息展示。

当前 UI 只在这些场景显式告诉用户定位缺失：

- 当前筛选周期没有任何 murmur：显示“还没有可浏览的碎碎念”。
- 当前筛选周期有 murmur，但 image/text observation 总数为 0：显示“还没有带定位的内容”，并提示有多少条内容没有经纬度。

如果周期内既有可地图化内容，也有未定位内容，地图只展示可用内容，不再额外显示“未定位 N 条”来打断浏览。
