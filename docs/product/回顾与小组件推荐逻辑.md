...
| `today-three-lines` | 三行里的今天 | 把一个片刻写成小诗 |
...
主要调用方是 Review 页面和 snapshot 刷新链路。App 进入前台、保存日记、跨天、同步拉到远端更新，以及原生小组件请求更新，都会通过 `journalEffects` 进入 snapshot 刷新链路。
...
| 原生小组件请求更新 | `refreshForWidgetUpdate` | 系统小组件更新时主动刷新 snapshot |
