...
| 主题 | 决策 |
| --- | --- |
| 仓库形态 | 已采用 pnpm workspace Monorepo |
| 当前重点 | 桌面端、移动端、`journal-core`、`journal-sync`、`journal-theme` 已拆入 workspace；继续补强移动端真机回归、媒体体积控制和导入导出 |
| 包管理 | 使用 pnpm workspace，并通过 `node-linker=hoisted` 保持 Expo / React Native / Electron 工具链兼容 |
| 共享范围 | 共享数据模型、Markdown 解析/序列化、Git 同步核心、同步调度、合并策略和主题 token；不共享桌面/移动 UI |
| 移动端技术栈 | Expo SDK 56 + React Native + TypeScript，真机调试和 E2E 默认使用 development build |
...
冲突阻断、处理入口和无共同祖先修复的后续需求见 [Git 同步冲突处理需求](./Git%20同步冲突处理需求.md)。
...
- 本机没有连接 Android 设备或模拟器，所以这次没有在 development build UI 上手动点击“同步”按钮。
