...
Expo 官方有 `expo-widgets`，但当前官方文档主要覆盖 iOS widgets、Live Activities 和相关 target 能力。验证这类原生能力需要 development build 或 EAS Build。
...
因此，如果要在 Expo 项目里做 Android 小组件，技术前提是接受 development build / EAS Build。
...
- 小组件能力只在 development build / EAS Build 中验收。
...
- `npm --workspace @journal/mobile run typecheck`
- `npm --workspace @journal/mobile run test`
...
| 风险 | 影响 | 缓解 |
| --- | --- | --- |
| Widget 需要 development build 调试 | 本地调试成本上升 | 把 widget 逻辑拆成可单测的 snapshot 生成函数 |
