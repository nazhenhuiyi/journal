...
| Sync core GitHub | push/clone、clean、merge、true conflict、`keep-local` / `keep-both` / `keep-remote`、受管文件首批矩阵 | 更复杂的文件类型组合和大仓库路径 |
| Mobile artifact | 冷启动、长文、碎碎念新增/编辑、回顾、设置校验 | artifact run id 仍依赖构建时 env |
...
| 优先级 | 问题 | 下一步 |
| --- | --- | --- |
| P0 | mobile artifact run id 不能运行时注入 | 做 native launch config bridge，或构建时固定 E2E run id |
| P0 | mobile conflict flow 需要进入正式验收 | 把 `sync-conflict` 纳入需要时必跑清单 |
| P1 | App 层没有真实选边恢复闭环 | mobile/desktop 各补一个策略 smoke |
| P1 | GitHub 文件类型覆盖需要继续扩展 | 首批已覆盖 delete、media、reviews、annotations、manifest；后续补复杂组合和大仓库路径 |
