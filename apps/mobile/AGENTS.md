...
- 本机 iOS 模拟器必须使用 localhost host；所有 Expo 启动脚本都会固定使用移动端 workspace 的本地 Expo SDK 54 CLI，iOS 入口会固定 `127.0.0.1` 和 IPv4 优先，并复用已有 localhost Metro。不要从 monorepo 根目录直接运行根 `node_modules/.bin/expo`。
