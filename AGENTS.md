...
- 工作区包括 `apps/desktop`、`apps/mobile`、`packages/journal-core`、`packages/journal-sync`、`packages/journal-theme`。
- 不要把某个子项目的约定写回根目录，比如桌面端视觉验收、移动端模拟器验收、共享包同步细节等。
- 使用 pnpm workspace。运行单个项目命令时优先使用 `pnpm --filter <workspace-name> run <script>`。
- 测试使用 Vitest。完整校验从根目录运行 `pnpm test`；不要传 Jest 专用参数，比如 `--runInBand`。
